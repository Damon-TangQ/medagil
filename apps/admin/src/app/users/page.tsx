...
import type { getAdminUsersResponse } from '@medagil/api-client/admin';
...
export default function UsersPage() {
  ...
  const {
    ...
    isLoading,
    isError,
  } = useQuery<getAdminUsersResponse>({
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      {hasToken && users && (!users.items || users.items.length === 0) && (
        ...
        <div className="overflow-x-auto rounded-xl border bg-white p-4 shadow-sm">
          ...
          <div className="mt-4 flex items-center justify-between border-t pt-4">
            <div className="text-sm text-gray-500">
              显示 <span className="font-medium">1</span> 到 <span className="font-medium">{Math.min(10, users.items.length)}</span> 条，共 <span className="font-medium">{users.total}</span> 条记录
            </div>
            <div className="flex gap-2">
              <button
                type="button"
