...
| 规则文件 | 说明 | 生效范围 |
|----------|------|----------|
| **go** | api-service（Go + Gin）分层与 OpenAPI 同步 | services/api-service/**/*.go |
| **python** | ai-service（Python + FastAPI）分层与 uv | services/ai-service/**/*.py |
| **nextjs** | apps/web、apps/admin（Next.js、api-client） | apps/web、apps/admin |
| **taro** | apps/miniapp（Taro、Mobx、api-client/user） | apps/miniapp/**/*.ts,*.tsx |
